...
struct Graph
{
    ...
    struct Edge
    {
        int rev, from, to, cap;
        bool real;
        Edge(int r, int f, int t, int c, bool real) : rev(r), from(f), to(t), cap(c), real(real) {}
    };
    ...
    void addedge(int from, int to, int cap)
    {
        ...
        list[from].push_back(Edge(torev, from, to, cap, true));
        list[to].push_back(Edge(fromrev, to, from, 0, false));
    }
};
...
bool reachable(Graph &G, int s, int t, vector<bool> &seen)
{
    if (s == t)
    {
        return true;
...
int main()
{
    while (true)
    {
        ...
        // フォード・ファルカーソン法
        FordFulkerson ff;
        int max = ff.solve(G, s, t);
        int total = 0;

        for (int v = 0; v < ff.seen.size(); v++)
        {
            if (ff.seen[v] == false)
                continue;
            for (auto &e : G[v])
            {
                // vから出ている点でreal=false && cap=0の点
                // seenに含まれていない
                if (e.real == false && e.cap == 0 && ff.seen[e.to] == false)
                {
